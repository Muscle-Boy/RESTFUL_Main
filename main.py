...
def RESTFUL():
    if request.method == 'GET':                 # [IMPORTANT] By default, when the client first enters the URL, he is in fact sending a GET request
        ...
        response = requests.post(url="http://backend-service:5010/", data = input_text)      # [SUPER IMPORTANT] This sends the request to ANOTHER REST API to carry out the processing
