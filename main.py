...
def RESTFUL():
    if request.method == 'GET':                 # [IMPORTANT] By default, when the client first enters the URL, he is in fact sending a GET request
        ...
        response = requests.post(url="http://backend-service:5010/")      # [SUPER IMPORTANT] This sends the request to ANOTHER REST API to carry out the processing
        return render_template('index.html', content=response)       # "response.text" extracts the text data from the REST API RESPONSE
